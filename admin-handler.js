...
const INTASEND_PUBLIC_KEY =
  "ISPubKey_live_017c47ee-a538-4039-85f1-cbfb26023df3";
...
function loadTicketLogo() {
  return new Promise((resolve) => {
    ...
    if (
      existingLogo &&
      existingLogo.complete &&
      existingLogo.naturalWidth > 0
    ) {
...
async function _processAndGenerateTickets(
  pendingOrderStr,
  ...
  try {
    ...
    const container =
      spinnerContainer || document.querySelector(".payment-form-container");
    if (container) {
      container.innerHTML = `
    ...
    if (
      !saveResult.success ||
      !Array.isArray(saveResult.tickets) ||
      saveResult.tickets.length === 0
    ) {
    ...
    // Send emails in background (skip if webhook already processed them)
    if (!savedTickets[0].webhookProcessed) {
      sendMultiTicketEmail(
        savedTickets,
        orderData.email,
        orderData.guestEmails,
      ).catch((e) => {
        console.error("Error sending emails in background:", e);
      });
    } else {
      console.log(
        "Emails already sent by webhook, skipping frontend email sending.",
      );
    }
  } catch (err) {
    ...
    alert(
      err.message ||
        "There was an error generating your tickets. Please contact support at sapegalanairobi@gmail.com",
    );
  } finally {
...
function _showPendingOrderRecoveryBanner(pendingOrderStr) {
  ...
  document
    .getElementById("recoverTicketBtn")
...
window.addEventListener("pageshow", (event) => {
  ...
  if (
    ...
    const overlays = document.querySelectorAll(
      'div[class*="intasend"], div[id*="intasend"]',
    );
...
function initializePaymentPage() {
  ...
  const additionalNamesSection = document.getElementById(
    "additionalNamesSection",
  );
  const additionalNamesContainer = document.getElementById(
    "additionalNamesContainer",
  );
  ...
  const EARLY_BIRD_DEADLINE = new Date(
    LAUNCH_DATE.getTime() + 9 * 24 * 60 * 60 * 1000,
  );
...
async function handlePaymentSubmit(e) {
  ...
  async function finaliseOrder(paymentMethod) {
    ...
    try {
      ...
      // Send emails in background without blocking UI (skip if webhook already processed them)
      if (!savedTickets[0].webhookProcessed) {
  ...
  try {
    ...
    try {
      await fetch(
        "https://us-central1-sape-gala-2026.cloudfunctions.net/savePendingOrder",
        {
          method: "POST",
      ...
      console.warn(
        "⚠️ Could not save order to Firestore (will rely on localStorage):",
        saveErr.message,
      );
    }
    ...
    if (hiddenBtn) {
      ...
      hiddenBtn.dataset.redirect_url =
...
function displayAllTickets(tickets) {
  ...
  tickets.forEach((ticketData, index) => {
    ...
    ticketLogo.style.cssText =
      "display:block; margin:0 auto 0.5rem; object-fit:contain;";
    ticketElement.appendChild(ticketLogo);

    if (tickets.length > 1) {
      const ticketHeader = document.createElement("h4");
      ticketHeader.style.cssText =
        'font-family: "Playfair Display", serif; color: var(--gold); margin-bottom: 1rem; font-size: 1.2rem;';
    ...
    if (index < tickets.length - 1) {
      ...
      separator.style.cssText =
        "border-top: 1px solid var(--border-color); margin: 2rem 0;";
...
async function downloadSingleTicket(ticketData, qrIndex = 0) {
  ...
  const rightCenterX =
    section1Width +
    section2Width +
    (canvas.width - section1Width - section2Width) / 2;
  ...
  ctx.textAlign = "center";
  ctx.fillText(
    ticketLabel === "— VIP —" || ticketLabel === "— VVIP —"
      ? ticketLabel
      : "TICKET",
    rightCenterX,
    45,
...
async function sendMultiTicketEmail(tickets, primaryEmail, guestEmails = []) {
  ...
  try {
    const functionUrl =
      "https://us-central1-sape-gala-2026.cloudfunctions.net/sendTicketEmail";
